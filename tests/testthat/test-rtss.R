test_that("adding pev works", {
  example_site <- single_site(example_site, 1)
  interventions <- example_site$interventions
  interventions$coverage <- 0.5
  interventions$vaccine<- 'RTS,S'
  interventions$booster_coverage<- 0.8
  ...
  expect_equal(mean(p1$pev_epi_booster_coverage), 0.8)
  ...
  expect_equal(p1$pev_epi_coverages, interventions$coverage)
