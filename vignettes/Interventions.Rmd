...
### Description
Supported net types currently include: "pyrethroid_only", "pyrethroid_pbo", "pyrethroid_pyrrole". Net type introductions are informed by data from alliance for malaria prevention Allicance. Current available net types are: pyrethroid_only, pyrethroid_pbo and pyrethroid_pyrrole. Absent systematic, country data on mass campaign dates and locations we assume that mass campaigns occur on the
first day of the year and routine distributions occur quartely. Lower and upper bounds help to ensure that
...
⚠️ **Warning:** For ITNs to work when passed to `site::site_parameters()` you will need to specify a column: `site_file$interventions$itn$implementation$itn_input_dist` indicating the size of the model ITN distrbutions each year. This is **not** the same as ITN usage. Please call `stop_missing_itn_input_dist()` for more information.
...
`site_file$interventions$itn$itn_use`
...
the first day of the year (alligns with the MAP net online model).
...
`round` Spray round  
`spay_day_of_year` Day of year of spray round  
...
**Within sub-Saharan Africa:** The population at risk weighted mean IRS coverage estimates for each spatial unit are summarise from the malaria atlas project

**Outside of sub-Saharan Africa:** IRS coverage is much more heterogeneous outside of SSA and data are less systematicallycollected. As a result, there are strong assumptions associated with the historical
...
### Description
The population at risk weighted mean effective treatment coverage estimates for each spatial unit are summarised from the malaria atlas project. Note, as this metric descibes "effective" treatment, we implement specially parameterised drugs implemented inthe site file with efficacy = 1. For the proporiton of treatments that are an ACT, for SSA estimates by year are expanded by linear interpolation between data 
...
`peak_season` Rainfall seasonal peak day of the year  
`smc_min_age` Lower bound of SMC-eligability age range (days)  
`smc_max_age` Upper bound of SMC-eligability age range (days)  
...
### Decription
Can be age-based or hybrid (seasoal boosters)
